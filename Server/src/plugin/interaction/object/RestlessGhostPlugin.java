...
public final class RestlessGhostPlugin extends OptionHandler {
	...
	public boolean handle(Player player, Node node, String option) {
		...
		case "search":
			...
			case 15050:
				...
				break;
			case 15051:
				if (!player.getNeoQuestRepository().hasFinished("The Restless Ghost") && !player.getBank().containsItem(SKULL) && !player.getInventory().containsItem(SKULL)) {
	...
	private void toggleCoffin(final Player player, final GameObject object) {
		...
		if (open && !player.getNeoQuestRepository().hasFinished("The Restless Ghost")) {
			sendGhost();
		}
	}
	...
	private void searchAltar(final Player player, final GameObject object) {
		...
		if (player.getNeoQuestRepository().getStage("The Restless Ghost") != 30) {
		...
		if (!hasSkull) {
			...
			player.getNeoQuestRepository().setStage("The Restless Ghost", 40);
	...
	public static final class RestlessGhostNPC extends AbstractNPC {
		...
		public boolean isHidden(final Player player) {
			...
			return player.getNeoQuestRepository().hasFinished("The Restless Ghost") || (pl != null && player != pl);
		}
