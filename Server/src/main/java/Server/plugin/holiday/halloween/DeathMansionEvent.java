...
import core.game.content.holiday.HolidayType;
...
public class DeathMansionEvent extends HolidayEvent {
	...
	public void commenceDeath(Entity killer, Entity victim) {
		...
		GameWorld.getPulser().submit(new Pulse(4, reaper) {
			@Override
			public boolean pulse() {
				reaper.clear();
				return true;
	...
	public class MuncherDialogue extends DialoguePlugin {
		...
		public boolean open(Object... args) {
			npc = (NPC) args[0];
			interpreter.sendDialogues(player, FacialExpression.JOLLY, "Here, boy!");
			return true;
		}
		...
		public boolean handle(int interfaceId, int buttonId) {
			...
			case 2:
				if (buttonId == 1) {
					...
					GameWorld.getPulser().submit(new Pulse(a.getDuration(), player) {
						@Override
						public boolean pulse() {
					...
					GameWorld.getPulser().submit(new Pulse(anim.getDuration(), player) {
						@Override
						public boolean pulse() {
