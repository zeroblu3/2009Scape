...
public class HouseServantDialogue extends DialoguePlugin {
	...
	public boolean open(Object... args) {
		...
		if (!manager.hasHouse() && inHouse) {
			interpreter.sendDialogues(npc, npc.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "You don't have a house that I can work in.", "I'll be waiting here if you decide to buy a house.");
			stage = 100;
			return true;
		}
		if (!manager.hasServant()) {
			...
			if(player.getSkills().getLevel(Skills.CONSTRUCTION) >= type.getLevel()){
				interpreter.sendDialogues(npc, npc.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "You're not aristocracy, but I suppose you'd do. Do you", "want a good cook for " + type.getCost() + " coins?");
				stage = 0;
				return true;
			} 
			interpreter.sendDialogues(npc, npc.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "You need a Construction level of " + type.getLevel() + " and you must not", "currently have another person working for you", "in order to hire me.");
			stage = 100;
			return true;
		} else {
			...
			if (inHouse) {
				...
				if (servant.getItem().getAmount() > 0) {
					...
					interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "I have returned with what you asked me to", "retrieve.");
					stage = 150;
					return true;
				}
				interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "Yes, " + (player.getAppearance().isMale() ? "sir" : "ma'am") + "?", "You have " + (8 - servant.getUses()) + " uses of my services remaining.");
				stage = 50;
			} else if(npc.getId() != servant.getId()){
				interpreter.sendDialogues(npc, npc.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "You already have someone working for you.", "Fire them first before hiring me.");
				stage = 100;
			}
		}
		return true;
	...
	public boolean handle(int interfaceId, int buttonId) {
		...
		case 1:
			...
			break;
		case 2:
			interpreter.sendDialogues(npc, npc.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "Alright, " + (player.getAppearance().isMale() ? "sir" : "ma'am") + ". I can start work immediately.");
			stage++;
			break;
		case 3:
		...
		case 52:
			...
			case 1:
				if (!servant.getAttributes().containsKey("con:lastfetch")) {
					interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "I haven't recently fetched anything from the bank or", "sawmill for you.");
					stage = 50;
					break;
				} else {
			...
			case 3:
				if (type == ServantType.MAID || type == ServantType.RICK) {
					interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "I am unable to travel to the sawmill for you.");
					stage = 100;
					break;
				}
				interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "Hand the logs to me and I will take them to the", "sawmill for you.");
				stage = 100;
				break;
			case 4:
				stage = 100;
		...
		case 56:
			...
			case 2:
				if (type.getFood() == null) {
					interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "I don't know any recipes.");
					stage = 100;
					break;
				}
			...
			break;
		case 57:
			interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "Whatever you command.");
			stage = 50;
			break;
		case 58:
			if (player.getInventory().freeSlots() < 1) {
				interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "I would love to share my fine cooking with you,", "but your hands are currently full.");
				stage = 100;
				break;
			}
			...
			interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "Luckily for you, I already have some made. Here you", "go.");
			stage = 50;
			break;
		case 59:
			if (player.getInventory().freeSlots() < 1) {
				interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "I would love to share my fine cooking with", "you, but you have no space to take anything.");
				stage = 50;
				break;
			}
			if (!prereqs(player, null, false)) {
		...
		case 62:
			...
			break;
		case 75:
			interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "Very well. I will return to the Guild of the Servants", "in Ardougne if you wish to re-hire me.");
			stage++;
			break;
		case 76:
			end();
		...
		case 150:
			...
			if (amtLeft < 1 || servant.getItem() == null) {
				interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "I don't have any items left.");
				stage = 100;
				break;
			}
	...
	private boolean prereqs(final Player player, final Item item, boolean sawmill) {
		...
		if (servant.getItem().getAmount() > 0) {
			interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "You can't send me off again, I'm still holding some of", "your previous items.");
			return false;
		}
		return true;
	}
	...
	private void bankFetch(final Player player, final Item item) {
		...
		if (!player.getBank().containsItem(item)) {
			interpreter.sendDialogues(servant, servant.getId() == 4243 ? FacialExpression.HALF_GUILTY : null, "You don't seem to have any of those in the bank.");
			stage = 100;
			return;
		}
		end();
